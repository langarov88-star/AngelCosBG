export async function onRequestPost({ request, env }) {
  if (!env?.ACCESS_PASSWORD) return json({ error: "ACCESS_PASSWORD missing in env" }, 500);
  ...
  const exp = now + 12 * 60 * 60; // 12 часа (по-сигурно)
  ...
  return json({ token: `${payloadB64}.${sigB64}`, exp }, 200);
}
